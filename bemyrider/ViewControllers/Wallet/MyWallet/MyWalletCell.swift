...
class MyWalletCell: UITableViewCell {
    ...
    @IBOutlet weak var lblDateAns: UILabel!
    @IBOutlet weak var lblAmountAns: UILabel!
    
    @IBOutlet weak var bgView: UIView!{
        didSet{
            DispatchQueue.main.async {
                self.bgView.setRadius(10, borderWidth: 1.5, color: UIColor(red: 240.0/255.0, green: 241.0/255.0, blue: 241.0/255.0, alpha: 1.0))
...
//                    self.bgView.shadow()
    ...
    var isProviderWallet:Bool = false{
        didSet{
            setProviderRelatedData()
        }
    }
    ...
    var depositeData : DepositHistoryList?{
        didSet{
            loadDepositeData()
        }
    }
    ...
    func setProviderRelatedData() {
        if isProviderWallet{
            self.lblDate.text = "Requested Date".localized
        }
        else{
