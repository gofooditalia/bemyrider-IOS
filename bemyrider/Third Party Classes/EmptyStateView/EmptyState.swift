...
public protocol EmptyStateDelegate: AnyObject {
    func emptyState(emptyState: EmptyState, state: CustomState, didPressButton button: UIButton)
}
...
extension EmptyState {
    ...
    private func didPressActionButton(_ button: UIButton) {
        guard let state = state else { return }
        delegate?.emptyState(emptyState: self, state: state, didPressButton: button)
    }
}
